...
<form method="GET" action="" id="filterForm">
  <div class="row mb-3">
    ...
    <div class="col-lg-3">
      ...
      <select name="reader_name" class="form-control auto-submit">
        <option value="">All</option>
        <?php
        $query = "SELECT DISTINCT reader_name FROM tbl_reading WHERE reader_name IS NOT NULL AND reader_name <> ''";
        $result = $con->query($query);
        while ($row = $result->fetch_assoc()) {
          ?>
          <option value="<?php echo $row['reader_name']; ?>" <?php echo (isset($_GET['reader_name']) && $_GET['reader_name'] == $row['reader_name']) ? 'selected' : ''; ?>>
        ...
        <?php } ?>
            ...
            <table id="daily_collection_report" class="table table-borderless datatable">
              ...
              <tbody style="text-align: center;">
                <?php
                $query = "SELECT * FROM tbl_reading WHERE payment_status = 'collector' AND 1=1";
                    ...
                    if (!empty($_GET['reader_name'])) {
                        $query .= " AND reader_name = '".$_GET['reader_name']."'";
                    }
                ...
                <tr>
                  ...
                  <td style="text-align: center;"><?php echo ucwords(str_replace("_", " ", $row["category"])); ?></td>
...
materialsRow.getCell(7).border = {
    ...
    prepNameRow.getCell(1).value = '<?php echo htmlspecialchars($collector_username); ?>';
    prepNameRow.getCell(1).font = { size: 17, bold: true };
    positionRow.getCell(1).value = 'Collector - Disconnector';
...
for (let i = 0; i < rows.length; i++) {
    ...
    // Center all the items from A6:J41
    for (let i = 6; i <= 41; i++) {
...
workbook.xlsx.writeBuffer().then(function(buffer) {
    ...
    a.href = url;
    const date = new Date();
    const preparedBy = '<?php echo htmlspecialchars($collector_username); ?>';
