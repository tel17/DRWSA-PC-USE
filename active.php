...
<body>
    ...
    <div class="row">
        ...
        <div class="column column-right">
            ...
            <table class="table table-borderless" id="Customer_Manager_Report">
                ...
                <tbody>
                <?php
                        // Fetch only records where consumer_status is "ACTIVE"
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                // Assign status badge for ACTIVE consumers
    ...
    <button class="btn btn-danger btn-sm btn-xs" onclick="updateStatus(<?php echo $row['id']; ?>)">DISCONNECTED</button>
</td>
    ...
    Swal.fire({
        title: 'Are you sure?',
        text: "You are about to update the consumer status to DISCONNECTED!",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: 'update_disconnected_status.php', // Correct endpoint to update status
                type: 'POST',
                data: { id: id, status: 'DISCONNECTED' }, // Send 'DISCONNECTED' as status
                success: function(response) {
                    // Update status in the table
                    $('#status_' + id).html('<span class="badge bg-danger">DISCONNECTED</span>');
                    Swal.fire('Updated!', 'The consumer status has been updated to DISCONNECTED.', 'success').then(() => {
